...
'''hypeparameter'''
 # Si el operador está presionado
IS_CALC = False
 # Números de tienda
STORAGE = []
 # ¿Cuántos caracteres se pueden mostrar en el cuadro de visualización?
MAXSHOWLEN = 18
 # Número mostrado actualmente
CurrentShow = tkinter.StringVar()
CurrentShow.set('0')


'''Presione la tecla numérica (0-9)'''
...
'''Borrar todos los números en el cuadro de visualización actual'''
def clearCurrent():
	CurrentShow.set('0')


'''Eliminar el último número en el cuadro de visualización'''
def delOne():
	global IS_CALC
	if IS_CALC:
		CurrentShow.set('0')
...
'''Calcular corrección de respuesta'''
	...
	if len(result) > MAXSHOWLEN:
		if len(result.split('.')[0]) > MAXSHOWLEN:
			result = 'Overflow'
			 ...
			 # Redondea directamente sin considerar el redondeo
...
def Demo():
	...
	root.title('Calculator')
	 # Diseño
	# --Caja de texto
	label = tkinter.Label(root, textvariable=CurrentShow, bg='black', anchor='e', bd=5, fg='white', font=('Cursiva', 20))
	...
	# ----Despejado(Equivalente a reiniciar)
